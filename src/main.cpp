                            ...
                            const double dt )
{
	...
	{
        MPI_Barrier(MPI_COMM_WORLD);
		first_stage<num_r_steps, num_phi_steps>( T_previous_layer, T_current_layer, startPhi, endPhi, a, dt, dphi, dr, R );
		std::swap( T_current_layer, T_previous_layer );
		MPI_Barrier(MPI_COMM_WORLD);
		exchange( T_previous_layer, startPhi, endPhi, num_r_steps, phi_size, rank, num_pc );

		second_stage<num_r_steps, num_phi_steps>( T_previous_layer, T_current_layer, startR, endR, a, dt, dphi, dr, R );
		std::swap( T_current_layer, T_previous_layer );
        ...
        for (int i = 0; i < num_phi_steps; ++i)
        {
            ...
            for (int j = 0; j < num_r_steps; ++j)
            {
                if (l > 0)
                {
                    ...
                    if (std::abs(T_current_layer[num_phi_steps * i + j] -
                                 T_previous_layer[num_phi_steps * i + j]) > terr)
                    {
                        terr = std::abs(T_current_layer[num_phi_steps * i + j] -
        ...
        if (l % 1 == 0 && rank ==0 )
            std::cout << "Current nevyazka = " << err << std::endl;
...
constexpr std::size_t num_r_steps = 240;
constexpr std::size_t num_phi_steps = 240;
